...
export default function GovernancePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const [proposalsData, height] = await Promise.all([fetchProposals(), fetchCurrentHeight().catch(() => null)])
  ...
  return (
    <LayoutWrapper title="Governance" subtitle="Proposals & Upgrades">
      <div className="p-6">
        {/* Header */}
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Governance Proposals</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <StatCard label="Total Proposals" value={proposals?.proposals.length || 0} isLoading={isLoading} />
            <StatCard
              label="Upgrade Proposals"
              value={
                proposals?.proposals.filter((p) => p.messages?.some((m) => m['@type']?.includes('MsgSoftwareUpgrade')))
                  .length || 0
              }
              isLoading={isLoading}
              highlight
            />
            {currentHeight && (
              <StatCard label="Current Height" value={parseInt(currentHeight).toLocaleString()} isLoading={isLoading} />
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-dark-card rounded-lg shadow-lg p-6">
          {isLoading ? (
            ...
            <ProposalList proposals={proposals.proposals} showFilter={true} />
          ) : (
            <EmptyState />
          )}
        </div>
...
function StatCard({
  ...
  return (
    <div
      className={`px-4 py-3 rounded-lg min-w-0 ${
        highlight
...
function ErrorState({ error, onRetry }: { error: string; onRetry: () => void }) {
  return (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">Error Loading Proposals</h3>
...
function EmptyState() {
  return (
    <div className="text-center py-12">
      ...
      <p className="text-gray-500 dark:text-gray-400">There are no governance proposals on the network yet.</p>
    </div>
  )
}
