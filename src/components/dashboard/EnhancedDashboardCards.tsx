...
import { convertUvnaToVna, formatBlockHeight, formatInflationRate } from '@/lib/api'
import {
...
export default function EnhancedDashboardCards({
  ...
  const cards = [
    ...
    },
    {
      ...
      details: {
        ...
        'Passed Proposals':
          proposals?.proposals.filter((p) => p.status === 'PROPOSAL_STATUS_PASSED').length.toString() || '0',
        'Rejected Proposals':
          proposals?.proposals.filter((p) => p.status === 'PROPOSAL_STATUS_REJECTED').length.toString() || '0',
      },
    },
    {
      id: 'network',
      ...
      details: {
        ...
        Proposer: header?.result.header.proposer_address
          ? `${header.result.header.proposer_address.substring(0, 20)}...`
          : 'Unknown',
      },
    },
  ]
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card) => (
          ...
          >
            ...
            <div className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{card.value}</div>

            {/* Card Details - Position fixed to ensure visibility */}
            {getActiveCard() === card.id && (
              <div
                className="absolute left-0 right-0 bg-white dark:bg-dark-card p-6 border border-gray-200 dark:border-dark-border rounded-lg shadow-xl z-50 mt-2 top-full"
      ...
      <div className="bg-white dark:bg-dark-card rounded-lg shadow-lg p-6">
        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">Explore Verana Network</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Link
            href="/trust-registries"
            className="group bg-white dark:bg-dark-card rounded-lg border border-gray-200 dark:border-dark-border p-6 hover:border-verana-accent hover:shadow-lg transition-all duration-200"
          >
            <div className="flex items-center space-x-4 mb-4">
              <div className="w-12 h-12 bg-verana-accent/10 dark:bg-verana-accent/20 rounded-lg flex items-center justify-center">
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Trust Registries</h4>
                <p className="text-gray-500 dark:text-gray-400 text-sm">Explore and manage trust registries</p>
              </div>
            </div>
            <div className="flex items-center text-gray-600 dark:text-gray-300 group-hover:text-verana-accent transition-colors">
          ...
          >
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">DID Directory</h4>
                <p className="text-gray-500 dark:text-gray-400 text-sm">Explore decentralized identifiers</p>
              </div>
            </div>
            <div className="flex items-center text-gray-600 dark:text-gray-300 group-hover:text-verana-accent transition-colors">
          ...
          >
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Network Graph</h4>
                <p className="text-gray-500 dark:text-gray-400 text-sm">Visualize network connections</p>
              </div>
            </div>
            <div className="flex items-center text-gray-600 dark:text-gray-300 group-hover:text-verana-accent transition-colors">
      ...
      {validators && validators.validators.length > 0 && (
        <div className="bg-white dark:bg-dark-card rounded-lg shadow-lg p-6">
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Validator Network</h3>
          <div className="space-y-4">
            {validators.validators.map((validator, index) => (
              <div
                key={validator.operator_address}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-50 dark:bg-dark-surface rounded-lg gap-4"
              >
      ...
      {proposals && proposals.proposals.length > 0 && (
        <div className="bg-white dark:bg-dark-card rounded-lg shadow-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Recent Proposals</h3>
            <Link
              href="/governance"
