      ...
      <div className="bg-white dark:bg-dark-card rounded-xl shadow-lg p-6 border border-gray-200 dark:border-dark-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div
              className={`w-3 h-3 rounded-full ${connectionStatus.status === 'connected' ? 'bg-green-500' : connectionStatus.status === 'loading' ? 'bg-yellow-500 animate-pulse' : 'bg-red-500'}`}
            />
            ...
            <div>
              ...
              <p className="text-lg font-bold mt-1">
                {isLoading
                  ? '...'
                  : genesis
                    ? formatTotalSupply(genesis.result.genesis.app_state.bank.supply[0]?.amount || '0')
                    : 'N/A'}
              </p>
            </div>
            <div className="w-12 h-12 bg-white bg-opacity-20 rounded-lg flex items-center justify-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold mt-1">
                {isLoading
                  ? '...'
                  : genesis
                    ? formatInflationRate(genesis.result.genesis.app_state.mint.minter.inflation)
                    : 'N/A'}
              </p>
            </div>
            <div className="w-12 h-12 bg-white bg-opacity-20 rounded-lg flex items-center justify-center">
